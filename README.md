...
- `dataset_making.py` generates datasets of trajectories by rolling out trained PPO policies stored in the folder `policies`.
- `dit_utils.py` contains a variety of functions used by the DiT codes for training and evaluation.
- `environment.yml` contains all the packages and their version to create a conda environment where these codes can be run.
- `eval_DiT.ipynb` and `eval_DiT.py` are similar codes to evaluate a pre-trained DiT.
- `hopper.py` is a modification of the [Hopper Gymnasium environment](https://gymnasium.farama.org/environments/mujoco/hopper/) including some useful functions like `reset_to` allowing to reset the environment at a desired state.
- `normalization.py` scales states and rewards for PPO.
- `plots.py` allows to plot and compare Hopper trajectories.
- `ppo_utils.py` has the `ReplayBuffer`, training, evaluation, saving, and loading functions for PPO.
- `projectors_SA.py` contains the different projectors introduced in DDAT to make the trajectories generated by diffusion actually *feasible* by respecting the robot dynamics.
- `train_DiT.py` and `train_ppo.py` are the scripts used to train respectively DiT and PPO policies.
