...
def open_loop_stats(env, planner, mID, N_s0=100, s0=None, H=300, N_samples=4, attr=None, N:int=None):
    ...
    for s_id in range(N_s0):
        ID_traj, _, reward_samples[s_id], _ = mID.closest_admissible_traj(Trajs[s_id])
...
def barplot_comparison(labels, list_of_survival, list_of_rewards,
    ...
    ax.set_title('Hopper reward ' + title)
    ...
    ax.set_title('Hopper survival ' + title)
    ax.tick_params(bottom=False, axis='x', labelrotation=90) # no ticks at the bottom, only the labels
    if save_as_svg is not None:
...
#%% Loading a dataset of trajectories generated by pretrained PPO models
